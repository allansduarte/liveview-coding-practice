defmodule LiveViewStudioWeb.PaginateLiveTest do
  ...
  test "clicking next, previous, and page links patch the URL", %{conn: conn} do
    ...
    assert_patched(view, "/paginate?page=2&per_page=1&sort_by=item&sort_order=asc")

    view
    |> element("a", "Previous")
    |> render_click()

    assert_patched(view, "/paginate?page=1&per_page=1&sort_by=item&sort_order=asc")
  ...
  test "changing the per-page form patches the URL", %{conn: conn} do
    ...
    assert_patched(view, "/paginate?page=2&per_page=10&sort_by=item&sort_order=asc")
  end
end

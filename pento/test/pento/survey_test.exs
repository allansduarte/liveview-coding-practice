defmodule Pento.SurveyTest do
  ...
  describe "demographics" do
    ...
    test "list_demographics/0 returns all demographics", ctx do
      demographic = demographic_fixture(user_id: ctx.user.id)
      assert Survey.list_demographics() == [demographic]
    end

    test "get_demographic!/1 returns the demographic with given id", ctx do
      demographic = demographic_fixture(user_id: ctx.user.id)
      assert Survey.get_demographic!(demographic.id) == demographic
    end

    test "create_demographic/1 with valid data creates a demographic", ctx do
      valid_attrs = %{
        gender: "male",
        year_of_birth: 1992,
      ...
      assert demographic.year_of_birth == 1992
      assert demographic.education_level == "bachelor’s degree"
    end
    ...
    test "update_demographic/2 with valid data updates the demographic", ctx do
      demographic = demographic_fixture(user_id: ctx.user.id)
      update_attrs = %{gender: "female", year_of_birth: 1993}

      assert {:ok, %Demographic{} = demographic} =
               Survey.update_demographic(demographic, update_attrs)

      assert demographic.gender == "female"
      assert demographic.year_of_birth == 1993
    end

    test "update_demographic/2 with invalid data returns error changeset", ctx do
      demographic = demographic_fixture(user_id: ctx.user.id)
    ...
    test "delete_demographic/1 deletes the demographic", ctx do
      demographic = demographic_fixture(user_id: ctx.user.id)
    ...
    test "change_demographic/1 returns a demographic changeset", ctx do
      demographic = demographic_fixture(user_id: ctx.user.id)
  ...
  describe "ratings" do
    ...
    test "list_ratings/0 returns all ratings", ctx do
      product = product_fixture()
      rating = rating_fixture(user_id: ctx.user.id, product_id: product.id)
      assert Survey.list_ratings() == [rating]
    end

    test "get_rating!/1 returns the rating with given id", ctx do
      product = product_fixture()
      rating = rating_fixture(user_id: ctx.user.id, product_id: product.id)
      assert Survey.get_rating!(rating.id) == rating
    end

    test "create_rating/1 with valid data creates a rating", ctx do
      product = product_fixture()
      valid_attrs = %{stars: 4, user_id: ctx.user.id, product_id: product.id}
    ...
    test "update_rating/2 with valid data updates the rating", ctx do
      product = product_fixture()
      rating = rating_fixture(user_id: ctx.user.id, product_id: product.id)
    ...
    test "update_rating/2 with invalid data returns error changeset", ctx do
      product = product_fixture()
      rating = rating_fixture(user_id: ctx.user.id, product_id: product.id)
    ...
    test "delete_rating/1 deletes the rating", ctx do
      product = product_fixture()
      rating = rating_fixture(user_id: ctx.user.id, product_id: product.id)
    ...
    test "change_rating/1 returns a rating changeset", ctx do
      product = product_fixture()
      rating = rating_fixture(user_id: ctx.user.id, product_id: product.id)
